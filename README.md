...
| `blurredLocation.setZoomByPrecision(precision)` | Zooms map to the given precision. | `blurredLocation.setZoomByPrecision(2)` This would zoom the map so that the precision of map becomes 2, and each grid square is `0.01` degrees wide and tall. |
| `blurredLocation.getZoomByPrecision(precision)` | Returns the correct zoom level for the given precision. Does not change the map. | `blurredLocation.setZoomByPrecision(0)` would return 6 |
